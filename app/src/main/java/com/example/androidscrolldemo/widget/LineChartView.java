...
import com.example.androidscrolldemo.R;
...
public class LineChartView extends View {
    ...
    private int dp2 = dpToPx(12);   //y轴文本距离左边
    private int dp3 = dpToPx(30); //x轴文本距离底边
    private int dp4 = dpToPx(10); //文本距离X轴和Y轴的距离
    private float xLength = 0;   //X坐标长度
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        if (changed) {
            ...
            width = getWidth();  //得到View的宽度
            height = getHeight(); //得到View高度
            initOnLayout();
    ...
    private void initOnLayout() {
        ...
        //得到y轴文本最大的宽度【如果文字最大宽度确认，则可以屏蔽下面代码】
        ...
        xOri = (int) (dp2 + textYWdith + dp4 + xylinewidth);//dp2是y轴文本距离左边，以及距离y轴的距离
        //X轴文本最大高度
        xValueRect = getTextBounds("000", xyTextPaint);
        float textXHeight = xValueRect.height();
        //得到x轴文本最大的高度 和 X轴绘制文本的矩阵
        ...
        yOri = (int) (height - dp4 - textXHeight - dp3 - xylinewidth);//dp3是x轴文本距离底边，dp2是x轴文本距离x轴的距离
        xInit = interval + xOri;
    ...
    private void drawXY(Canvas canvas) {
        ...
        for (int i = 0; i < yValue.size(); i++) {
            ...
            canvas.drawText(text, 0, text.length(), xOri - xylinewidth - dp4 - rect.width(), yOri - yLength * i + rect.height() / 2, xyTextPaint);
        }
        //绘制X轴坐标
        ...
        path = new Path();
        //整个X轴的长度
        xLength = xInit + interval * (xValue.size() - 1) + (width - xOri) * 0.1f;
        if (xLength < width){
            xLength = width;
        }
        ...
        for (int i = 0; i < xValue.size(); i++) {
            ...
            if (x >= xOri) {//只绘制从原点开始的区域
                ...
                if (i == selectIndex - 1) {
                    ...
                    /**
                     * @param text  要绘制的文本
                     * @param start 要绘制的文本中的第一个字符的索引
                     * @param end   是要绘制的文本中的最后一个字符的索引
                     * @param x     x正在绘制的文本的原点的x坐标
                     * @param y      y正在绘制的文本的基线的y坐标
                     * @param paint  用于文本的颜色（例如颜色，大小，样式）
                     */
                    canvas.drawText(text, 0, text.length(), x - rect.width() / 2, yOri + xylinewidth + dp4 + rect.height(), xyTextPaint);
                    //从左上角开始绘制
                    canvas.drawRoundRect(x - xValueRect.width() / 2 - dpToPx(3),
                            yOri + xylinewidth + dp4,
                            x + xValueRect.width() / 2 + dpToPx(3),
                            yOri + xylinewidth + dpToPx(2) + xValueRect.height() + dpToPx(2) + dp4,
                            dpToPx(2),
                            dpToPx(2),
                            xyTextPaint);
                } else {
...
//                if (interval * xValue.size() > width - xOri) {//当期的宽度不足以呈现全部数据
                if (xLength > width - xOri) {//当期的宽度不足以呈现全部数据
